...
public class NameNodeResourceChecker {
  ...
  public NameNodeResourceChecker(Configuration conf) throws IOException {
    ...
    // 配置fsimage 的存放路径
    //localEditDirs -》 hdfs-site.xml core-site.xml 元数据存放在

    /**
     * dfs.namenode.edits.dir  存放 namenode 的事务文件 (edits) 的目录, 如果这是一个逗号分隔的目录列表, 那么事务文件在所有目录中被复制, 用于冗余. 默认与 dfs.namenode.name.dir 属性目录一样.
     *
     * dfs.namenode.edits.dir.required 是 dfs.namenode.edits.dir 属性目录的子集, 用于确保这些 edits 文件目录是最新的.
     */
   ...
   * is marked redundant 冗余, it will be marked required. If the volume is already
   * present but marked required then this method is a no-op. 不可执行
   * 
